...
class ParameterPool(object):
    ...
    def parameter_names(self):
        params = set()
        for param_name in self._pool.keys():
    ...
    def put(self, param, force = False):
        ...
        if not isinstance(param, Parameter):
            raise AttributeError(u"Cannot add item that's not instance of Parameter.")
    ...
    def has(self, name):
        return self._pool.has_key(name)
...
class ParameterValidator(object):
    ...
    def validate(self, parameter_pool, source = None):
        ...
        for name, parameter in parameter_pool.parameters.items():
